...
public class DataUtils {
    ...
    private static void getConnectList(List<alloperators> alloperatorses) {
        alloperatorses.add(new alloperators(i++, "connect()", "", "指示一个可连接的Observable开始发射数据", CommonString.connect, OperatorsUrl.connect, parentId));
        alloperatorses.add(new alloperators(i++, "publish()", "", "将一个Observable转换为一个可连接的Observable", CommonString.publish, OperatorsUrl.publish, parentId));
        alloperatorses.add(new alloperators(i++, "replay()", "", "确保所有的订阅者看到相同的数据序列，即使它们在Observable开始发射数据之后才订阅", CommonString.replay, OperatorsUrl.replay, parentId));
        alloperatorses.add(new alloperators(i++, "refCount()", "", "让一个可连接的Observable表现得像一个普通的Observable", CommonString.refCount, OperatorsUrl.refCount, parentId));
        parentId++;
    }
